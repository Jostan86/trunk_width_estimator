...
m2f_path = os.environ.get('M2F_PATH')

sys.path.insert(1, m2f_path)
...
config_file = m2f_path + "/configs/coco/instance-segmentation/maskformer2_R50_bs16_50ep.yaml"
weight_file = data_path + "/output20220612_5instances/model_final.pth"
...
for d in ["train", "val"]:
    ...
    image_root = os.path.join(data_path, "train_rgb")
    ...
    instances_json = os.path.join(data_path, "tree_trunk_ann_instance_train.json")
...
cfg.DATASETS.trial  = ("treepruning2_val",)
...
def trial(predictions, score_thresh,v,image):
...
import pandas as pd
def getwidths3(row):
    df=pd.read_csv(data_path + 'trunk_gt/row_{}_diameters.csv'.format(row))
    # if row!=96:
...
def trial2(predictions, score_thresh,v,image):
    boxes = predictions.pred_boxes if predictions.has("pred_boxes") else None
    scores = predictions.scores if predictions.has("scores") else None
    classes = predictions.pred_classes if predictions.has("pred_classes") else None
    keypoints = predictions.pred_keypoints if predictions.has("pred_keypoints") else None
...
def main_trial(row_num, trial_num2):
    ...
    save_root_image = dataset_path + "results/original_method/row{}/images/measure{}/".format(row_num, trial_num2)
    check_mkdir(save_root_image)

    save_root_mask = dataset_path + "results/original_method/row{}/masks/measure{}/".format(row_num, trial_num2)
    check_mkdir(save_root_mask)

    save_path_widths = dataset_path + "results/original_method/row{}/widths/".format(row_num)
    check_mkdir(save_path_widths)
    save_path_widths += "widths{}".format(trial_num2)

    save_path_plot = dataset_path + "results/original_method/row{}/plots/".format(row_num)
    check_mkdir(save_path_plot)
    save_path_plot += "widths{}.png".format(trial_num2)

    training_sample_path = dataset_path + "row"+str(row_num)

    truewidths=getwidths3(row_num)
    ...
    #     print(img_path)
        ...
        vis_imgs2,mask,scores,x,y =trial2(outputs["instances"].to("cpu"),0.2,v,im)
        if scores!=-1:
        ...
        w5[info2-1]=a.width5
        save_path_image = os.path.join(save_root_image, data_sample)
        save_path_mask = save_root_mask + info +'.npy'
        cv2.imwrite(save_path_image,vis_imgs2)
        np.save(save_path_mask, mask)
        # break

    linex=[min_t,max_t]
    liney=[min_t,max_t]
    # saved_folder_path3="/home/jostan/Downloads/datasets_og/tests/" + "row{}_3".format(trial_num)

    np.save(save_path_widths, w5)
